  ...
  useEffect(() => {
    const loadVendors = async () => {
      ...
      try {
        ...
        toasts.add({ title: 'Falha ao carregar os serviços disponíveis', variant: 'error' })
      } finally {
  ...
  return (
    <div style={style}>
      <div className="overflow-y-auto" style={{ maxHeight }}>
        {!ready ? (
          <p className={PICKER_EMPTY}>Carregando conexões…</p>
        ) : matchedResources.length === 0 ? (
          <p className={PICKER_EMPTY}>Nenhum recurso encontrado.</p>
        ) : (() => {
          let itemIdx = 0
              ...
              <div key={`${vendor.id}-${resource.urlPattern}`} className={i > 0 ? 'border-t border-kumo-line' : ''}>
                ...
                {vendorAccounts.map(account => {
                    ...
                    >
                      ...
                      {isReconnecting || isGranting ? (
                        ...
                        <span className="flex flex-shrink-0 items-center gap-1">
                          ...
                          <span className="text-[11.5px] leading-4 text-kumo-warning">Expirado — clique para reautenticar</span>
                        </span>
                      ) : needsAccess ? (
                        <span className="flex flex-shrink-0 items-center gap-1">
                          <Warning size={12} className="text-kumo-warning" />
                          <span className="text-[11.5px] leading-4 text-kumo-warning">Conceder acesso</span>
                        </span>
                  ...
                  if (searchHasPlaceholders) {
                    return (
                      <Tooltip key={account.id} content="Substitua todos os espaços reservados na URL antes de selecionar uma conta" asChild>
                        {accountRow}
                      </Tooltip>
                    )
                  }
                ...
                {(() => {
                  ...
                  >
                    ...
                    <span className="flex-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
                      {connectingVendor === vendor.id ? 'Abrindo…' : 'Conectar nova conta'}
                    </span>
                    {isActive && <TabHint />}
                  </div>
                  )
