...
function buildOptions(gatewayMode: boolean, enabledProviders: Set<string> | null) {
  ...
  for (const provider of providerOrder) {
    ...
    options.push({
      ...
      label: `Outro ${PROVIDER_LABELS[provider] || provider}...`,
      provider,
    })
  }
...
export default function AddModelModal({ visible, onCancel, onSuccess, authenticatedApi, aiConfig }: AddModelModalProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!selection) {
      newErrors.selection = gatewayMode ? 'Selecione um provedor' : 'Selecione um modelo'
    }

    if (selection?.type === 'custom') {
      if (!modelId.trim()) newErrors.modelId = 'Informe o ID do modelo'
      if (!displayName.trim()) newErrors.displayName = 'Informe um nome de exibição'
    }
    ...
    if (showCredentials && selection && !isOllama && !apiToken.trim()) {
      newErrors.apiToken = 'Informe seu token de API'
    }

    if (showCredentials && isCloudflare && !accountId.trim()) {
      newErrors.accountId = 'Informe o ID da sua conta Cloudflare'
    }

    if (showCredentials && isOllama && !apiUrl.trim()) {
      newErrors.apiUrl = 'Informe a URL da API do Ollama'
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Modelo de IA adicionado com sucesso', variant: 'success' })
      onSuccess()
    } catch (error: any) {
      console.error('Failed to add model:', error)
      toasts.add({ title: 'Falha ao adicionar modelo', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
    ...
    <Dialog.Root open={visible} onOpenChange={(open) => { if (!open) onCancel() }}>
      <Dialog className="p-6" size="lg">
        ...
        <div className="space-y-4">
          ...
          <Select
            label={gatewayMode ? 'Selecionar Provedor' : 'Selecionar Modelo'}
            className="w-full text-sm"
            placeholder={gatewayMode ? 'Escolha um provedor...' : 'Escolha um modelo de IA...'}
            value={selectValue}
          ...
          {showCustomFields && (
            <>
              <Input
                label="ID do modelo"
                placeholder={`ex.: ${example!.modelId}`}
                description={`O identificador do modelo conforme especificado pelo provedor (ex.: '${example!.modelId}')`}
                value={modelId}
              ...
              <Input
                ...
                description="Nome legível exibido na interface"
                value={displayName}
          ...
          {showCredentials && isCloudflare && (
            <Input
              ...
              description="A conta Cloudflare que será cobrada pelo uso do Workers AI"
              value={accountId}
          ...
          {showCredentials && selection && (
            <SensitiveInput
              ...
              description={
                isOllama
                  ? 'Opcional para acesso local ao Ollama'
                  : isCloudflare
                  ? 'Um token de API com permissões de Leitura + Edição do Workers AI (no painel: Workers AI > Use REST API > Create a Workers AI API Token)'
                  : `Seu token de API da ${PROVIDER_LABELS[selection.provider]} para cobrança`
              }
              value={apiToken}
          ...
          {showCredentials && isOllama && (
            <Input
              ...
              description="URL do seu servidor Ollama"
              value={apiUrl}
          ...
          {showCredentials && selection && !isOllama && !isCloudflare && (
            ...
            >
              <Collapsible.DefaultTrigger>Configurações avançadas</Collapsible.DefaultTrigger>
              <Collapsible.DefaultPanel>
                <Input
                  label="URL da API"
                  placeholder="https://..."
                  description="Substitui o endpoint padrão da API (útil para proxies como o Cloudflare AI Gateway)"
                  value={apiUrl}
