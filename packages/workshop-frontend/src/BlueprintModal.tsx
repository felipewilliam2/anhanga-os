...
function canvasToBlob(canvas: HTMLCanvasElement, type: string, quality: number): Promise<Blob> {
  return new Promise((resolve, reject) => {
    canvas.toBlob(blob => {
      ...
      else reject(new Error('Falha ao codificar a imagem.'))
    }, type, quality)
  })
}
...
export default function BlueprintModal({ open, onClose, overseer, gadget, metadata }: Props) {
  ...
  const loadBlueprints = useCallback(async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Falha ao carregar templates', variant: 'error' })
    } finally {
      setLoading(false)
    }
  }, [overseer])
  ...
  const loadBindings = useCallback(async () => {
    ...
    try {
      ...
      setBindingsError('Não foi possível carregar as conexões.')
    } finally {
  ...
  const handleScreenshotSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toasts.add({ title: 'Falha ao processar a captura de tela', variant: 'error' })
    } finally {
  ...
  const createBlueprint = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Template criado.', variant: 'success' })
      setFormMode('list')
      setNewTitle(metadata.title)
      setNewDescription('')
      setNewScreenshotBlob(null)
      ...
      setCreateError(err.message || 'Não foi possível criar o template.')
    } finally {
      setCreating(false)
    }
  }
  ...
  const saveBlueprintEdits = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Template atualizado.', variant: 'success' })
      setFormMode('list')
      ...
      setCreateError(err.message || 'Não foi possível atualizar o template.')
    } finally {
      setCreating(false)
    }
  }
  ...
  const deleteBlueprint = async (id: string) => {
    ...
    try {
      ...
      toasts.add({ title: err.message || 'Falha ao excluir o template.', variant: 'error' })
    } finally {
      setDeletingId(null)
    }
  }
          ...
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-4 py-5 sm:px-6">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <Dialog.Title className="text-[17px] leading-6 font-medium tracking-[-0.35px] text-kumo-default">
                {formMode === 'create' ? 'Criar template' : formMode === 'edit' ? 'Editar template' : 'Templates'}
              </Dialog.Title>
              <Dialog.Description className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                {formMode === 'create'
                  ? 'Descreva o que as pessoas obtêm ao começar a partir deste template.'
                  : formMode === 'edit'
                    ? 'Atualize os detalhes, a captura de tela e as orientações de conexão deste template.'
                    : 'Transforme este gadget em um ponto de partida reutilizável.'}
              </Dialog.Description>
              </div>
            </div>
            <Dialog.Close
                    ...
                    <div className="rounded-xl border border-kumo-line bg-kumo-base p-3">
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            Imagem opcional exibida no Explorar e na página de detalhes do template.
                            {formMode === 'edit' && !newScreenshotUrl && editingBlueprint?.screenshotUrl && !clearScreenshot ? ' A captura de tela atual permanecerá, a menos que você envie uma nova.' : ''}
                          </p>
                        </div>
                        <div className="flex shrink-0 items-center gap-1.5">
                          ...
                          >
                            ...
                            {processingScreenshot ? 'Processando...' : newScreenshotUrl || (formMode === 'edit' && editingBlueprint?.screenshotUrl && !clearScreenshot) ? 'Alterar' : 'Enviar'}
                          </WorkshopButton>
                        </div>
                      </div>
                      {screenshotPreviewUrl && (
                        <div className="mt-3 overflow-hidden rounded-lg border border-kumo-line bg-kumo-tint">
                          <img
                            ...
                            alt="Prévia da captura de tela do template"
                      ...
                      {clearScreenshot && !newScreenshotUrl && (
                        <div className="mt-3 rounded-lg border border-dashed border-kumo-line bg-kumo-tint px-3 py-2 text-[12px] leading-4 text-kumo-subtle">
                          A captura de tela será removida ao salvar.
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {bindingsLoading ? (
                    ...
                    <section>
                      ...
                      <p className="m-0 mb-3 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                        Nomeie cada conexão e adicione orientações para as pessoas que usarem este template.
                      </p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {creating
                        ...
                        : processingScreenshot ? 'Processando...'
                          : bindingsLoading ? 'Carregando...'
              ...
              >
                <span>
                  ...
                  <span className="mt-0.5 block text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                    Publique este gadget como um template reutilizável.
                  </span>
                </span>
                    ...
                    <BlueprintRow
                      ...
                      onUpdateCode={async () => {
                        try {
                          ...
                          toasts.add({ title: 'Template atualizado para o código atual.', variant: 'success' })
                          loadBlueprints()
                        } catch (err: any) {
                          toasts.add({ title: err.message || 'Falha ao atualizar o template.', variant: 'error' })
                        }
                      }}
                      onRetryPublish={async () => {
                        try {
                          await overseer.retryBlueprintPublish(bp.id)
                          toasts.add({ title: 'Template publicado com sucesso.', variant: 'success' })
                          loadBlueprints()
                        } catch (err: any) {
                          toasts.add({ title: err.message || 'A nova tentativa falhou.', variant: 'error' })
                        }
                      }}
                      onCopyLink={async () => {
      ...
      >
        <div className="flex w-full flex-wrap items-center gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              Pessoas que criaram um gadget a partir deste template não serão afetadas, mas o link deixará de funcionar.
            </p>
          </div>
          <button
            type="button"
    ...
    >
      <div className="flex min-w-0 flex-wrap items-center gap-3">
        ...
        <span
          ...
          title={bp.dirty ? 'Última publicação falhou' : undefined}
        >
          {bp.dirty && (
            <span
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <div className="-ml-[7px] flex flex-wrap items-center gap-1">
          ...
          >
            {copyState === 'copied' ? 'Copiado' : copyState === 'failed' ? 'Falha ao copiar' : 'Copiar link'}
          </GhostButton>
        </div>
