      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <h3 className="truncate text-sm font-medium text-kumo-default">
            {item.title || 'Template sem título'}
          </h3>
        </div>
        {item.description && (
        ...
        <DropdownMenu>
          ...
          <DropdownMenu.Content className={MENU_CONTENT}>
            <DropdownMenu.Item onClick={() => onTogglePin(item)} className={MENU_ITEM}>
              ...
              {item.pinned ? 'Desfavoritar' : 'Favoritar'}
            </DropdownMenu.Item>
            {item.inLibrary && (
...
export default function BlueprintList() {
    ...
    Promise.all([authenticatedApi.listOwnBlueprints(), authenticatedApi.listLibraryBlueprints()])
      .then(([own, library]) => {
        ...
        const ensure = (id: string): BlueprintItem => {
          let it = map.get(id)
          if (!it) {
            it = { id, title: 'Template sem título', description: '', recency: 0, pinned: false, inLibrary: false, isOwn: false }
            map.set(id, it)
          }
          return it
        }
  ...
  const handleBlueprintSelected = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toasts.add({ title: 'Template enviado', variant: 'success' })
      load()
    } catch (err) {
      console.error('Failed to upload blueprint:', err)
      toasts.add({ title: 'Falha ao enviar template', variant: 'error' })
    } finally {
      setUploading(false)
    }
  ...
  const handleRemoveFromLibrary = async (item: BlueprintItem) => {
    try {
      ...
      toasts.add({ title: 'Removido da biblioteca', variant: 'success' })
    } catch (err) {
      console.error('Failed to remove blueprint from library:', err)
      toasts.add({ title: 'Falha ao remover template', variant: 'error' })
    }
  }
        ...
        <div className="mb-4 flex items-center gap-2 px-3">
          ...
          <div className="grid shrink-0 grid-cols-2 gap-2">
            ...
            <button
              ...
              disabled={uploading}
              title="Enviar um arquivo .gadget"
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">Algo deu errado ao carregar seus templates.</p>
            <button type="button" onClick={load} className="mt-1 text-kumo-brand underline">Tentar novamente</button>
          </div>
        ) : filtered.length === 0 ? (
          search ? (
            <div className="py-12 text-center text-sm text-kumo-inactive">Nenhum template encontrado</div>
          ) : (
            <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
              ...
              <div>
                <p className="text-sm font-medium text-kumo-default">Nenhum template ainda</p>
                <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                  Publique um espaço de trabalho como template, ou adicione um a partir do Explorar.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  <UploadSimple size={14} weight="bold" />
                  {uploading ? 'Enviando…' : 'Enviar .gadget'}
                </button>
              </div>
            </div>
          )
