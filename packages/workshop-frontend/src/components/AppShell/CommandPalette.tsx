...
function mergeBlueprints(
  ...
  for (const b of library) {
    map.set(b.id, {
      id: b.id,
      title: b.metadata.title || 'Template sem título',
  ...
  for (const b of own) {
    ...
    map.set(b.id, {
      id: b.id,
      title: b.title || prev?.title || 'Template sem título',
...
export default function CommandPalette({
  ...
  const { groups, flat } = useMemo(() => {
    ...
    const wsBase: Command[] = gadgets
      ...
      .map((g) => ({
        id: `ws-${g.id}`,
        label: g.title || 'Espaço de trabalho sem título',
    ...
    const built: Group[] = searching
      ...
      : [
          { heading: 'Ações', items: refine(nav, nav.length) },
          { heading: 'Espaços de trabalho recentes', items: refine(wsBase, 4) },
        ]
      ...
      <div className="themed-floating-shadow-lg relative w-full max-w-xl overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
        <div className="flex items-center gap-2.5 border-b border-kumo-line px-3.5">
          ...
          <input
            ...
            onKeyDown={onKeyDown}
            placeholder="Buscar espaços de trabalho e ações…"
        ...
        <div ref={listRef} className="sidebar-scroll max-h-[min(60vh,420px)] overflow-y-auto p-1.5">
          {flat.length === 0 ? (
            <p className="px-3 py-6 text-center text-[13px] text-kumo-inactive">Nenhum resultado.</p>
          ) : (
            groups.map((group, gi) => {
