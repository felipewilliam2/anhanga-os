...
export default function GatekeeperAppPage({ appId }: { appId: string }) {
  ...
  useEffect(() => {
    ...
    authenticatedApi
      ...
      .then((frame) => {
        if (!frame) {
          if (!cancelled) setError('Este app não está disponível nesta implantação.')
          return
        }
        if (cancelled) {
          disposeFrame(frame)
  ...
  if (!state) {
    return <div className="px-4 py-16 text-center text-sm text-kumo-subtle">Carregando…</div>
  }
