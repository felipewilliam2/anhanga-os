  ...
  return (
    ...
    >
      <button
        ...
        title={expanded ? 'Recolher saídas' : 'Expandir saídas'}
        aria-label={expanded ? 'Recolher saídas' : 'Expandir saídas'}
      ...
      >
        {expanded && (
          <span className="text-[11px] font-medium uppercase tracking-[0.06em]">Saídas</span>
        )}
      ...
      <div className="flex flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden px-1.5 pb-2">
        {gadgets.map(gadget => {
          ...
          return (
            ...
            >
              <Tooltip content={`${gadget.title}${!expanded && isPending ? ' (Rascunho)' : ''}${hasHook ? ' · Hooks habilitados' : ''}`} asChild>
                <button
                  type="button"
              ...
              {expanded && (
                <WorkshopIconButton
                  ...
                  title="Renomear gadget"
                  aria-label={`Renomear ${gadget.title}`}
                >
        ...
        <Tooltip content="Ver atividade" asChild>
          <button
            type="button"
          ...
          >
            ...
            {expanded && <span className="min-w-0 flex-1 truncate">Ver atividade</span>}
            {expanded ? (
