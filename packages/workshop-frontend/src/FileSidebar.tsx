...
export default function FileSidebar({
  ...
  const handleCreateFile = () => {
    if (!newFileName.trim()) {
      toasts.add({ title: 'O nome do arquivo não pode ficar vazio', variant: 'error' })
      return
    }

    if (files.includes(newFileName.trim())) {
      toasts.add({ title: 'Já existe um arquivo com esse nome', variant: 'error' })
      return
    }
  ...
  const startDelete = (filename: string) => {
    if (files.length <= 1) {
      toasts.add({ title: 'Não é possível excluir o último arquivo restante', variant: 'error' })
      return
    }
  ...
  return (
    <div className="flex h-full w-[244px] flex-col border-r border-kumo-line bg-kumo-base">
          ...
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                Crie um novo arquivo neste gadget.
              </Dialog.Description>
            </div>
            <Dialog.Close
              render={(props) => (
      ...
      <DeleteConfirmationDialog
        ...
        description={<>Isso remove <span className="font-mono text-kumo-default">{deletingFile}</span> do gadget. Essa ação não pode ser desfeita.</>}
        ...
        >
          ...
          {isStreamingActive && (
            <span
              ...
              aria-label={`${filename} está sendo editado`}
              title="O agente está editando este arquivo"
            />
          )}
        </button>
      )}
