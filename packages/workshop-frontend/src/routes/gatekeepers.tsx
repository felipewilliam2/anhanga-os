  ...
  const trailing =
    state === 'expired' && onReconnect ? (
      ...
      >
        ...
        {reconnectBusy ? 'Abrindo...' : 'Reconectar'}
      </button>
    ) : (
    ...
    <div className="relative isolate hidden h-[176px] w-[444px] lg:block">
      ...
      <div className="group absolute left-[176px] top-[58px] z-20">
        <button
          ...
          aria-label="O Guardião mantém o acesso do Gadget limitado aos recursos conectados"
        >
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[11px] leading-4 font-normal tracking-[-0.1px] text-kumo-subtle">
                Mantém cada espaço de trabalho limitado aos recursos que você conecta e garante que todo usuário tenha as permissões necessárias antes de acessá-los.
              </p>
            </div>
          </div>
...
function ConnectorsPage() {
    ...
    authenticatedApi
      ...
      .then((vendorList) => {
        ...
        if (unavailable.length > 0) {
          toasts.add({
            title: `Alguns serviços estão temporariamente indisponíveis: ${unavailable.map((v) => v.id).join(', ')}`,
            variant: 'warning',
          })
        }
        setVendors(
  ...
  const handleConfirmConnect = async (resourceUrlPatterns?: string[]) => {
    ...
    try {
      ...
      toasts.add({ title: 'Falha ao iniciar a conexão', variant: 'error' })
    } finally {
      setConnecting(false)
    }
  }
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Falha ao desconectar a conta', variant: 'error' })
    } finally {
  ...
  const handleReconnect = async (accountId: number) => {
    ...
    try {
      ...
      toasts.add({ title: 'Falha ao reconectar a conta', variant: 'error' })
    } finally {
      ...
      <div className="mx-auto w-full max-w-5xl px-4 py-12 sm:px-8 sm:py-14">
        <header className="mb-8 grid gap-8 lg:grid-cols-[minmax(0,540px)_444px] lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-[14px] leading-[20px] font-normal tracking-[-0.25px] text-kumo-subtle">
              Adicione os apps e contas que seus espaços de trabalho podem usar. Conecte uma vez e
              use-os em tudo o que você construir.
            </p>
          </div>
        ...
        {loadError && (
          <div className="rounded-2xl border border-kumo-line bg-kumo-base px-4 py-6 text-center">
            <p className="m-0 text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
              Algo deu errado ao carregar seus guardiões.
            </p>
        ...
        {filteredAccounts.length > 0 && (
          <section className="mb-10">
            <SectionEyebrow label="Conectados" count={filteredAccounts.length} />
        ...
        {filteredAvailable.length > 0 && (
          <section className="mb-10">
            <SectionEyebrow label="Disponíveis" />
            <div className={sectionGridClass}>
        ...
        {!initialLoading &&
          ...
          filteredAvailable.length === 0 && (
            <EmptyState
              title={
                search
                  ? 'Nenhum guardião corresponde'
                  : 'Nenhum guardião ainda'
              }
              description={
                search
                  ? 'Não encontramos nada correspondente à sua busca.'
                  : 'Os guardiões aparecerão aqui conforme ficarem disponíveis no seu espaço de trabalho.'
              }
              icon={Plugs}
            />
          )}
