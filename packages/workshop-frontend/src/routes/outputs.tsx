...
function subtitle(output: OutputSummary): string {
  const parts = [output.workspaceTitle || 'Espaço de trabalho sem título']
  if (output.owner) parts.push(`Compartilhado por ${output.owner.name}`)
  parts.push(`Espaço de trabalho ativo ${formatRelativeTime(output.lastActive)}`)
  return parts.join(' · ')
}
...
function OutputProvenance({ owner }: { owner?: OutputSummary['owner'] }) {
  return (
    <span
      ...
      title={owner ? `Em um espaço de trabalho compartilhado por ${owner.name}` : 'Em um espaço de trabalho que você criou'}
    >
      {owner ? <ShareNetwork size={11} /> : <User size={11} />}
      <span className="truncate">{owner ? `Compartilhado por ${owner.name}` : 'Criado por você'}</span>
    </span>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[12px] leading-4 tracking-[-0.2px] text-kumo-subtle">
          {formatOf(output.output).noun} · {output.workspaceTitle || 'Espaço de trabalho sem título'}
        </p>
      </div>
      ...
      <div className="hidden shrink-0 items-center gap-6 text-xs text-kumo-inactive lg:flex">
        ...
        <span className="flex w-40 items-center justify-end gap-1 whitespace-nowrap">
          <Clock size={10} />
          Espaço de trabalho ativo {formatRelativeTime(output.lastActive)}
        </span>
      </div>
      <OutputMenu onOpen={onOpen} onOpenWorkspace={onOpenWorkspace}
                  onRename={onRename} onRemove={onRemove} />
      ...
      >
        <form onSubmit={(event) => { event.preventDefault(); onSave() }}>
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
            <div className="min-w-0">
              ...
              <Dialog.Description className="mt-1 text-[12px] leading-4 text-kumo-subtle">
                Renomeia a saída para todos com acesso a “{output?.workspaceTitle}”.
              </Dialog.Description>
            </div>
            <WorkshopIconButton type="button" className="!h-7 !w-7" disabled={busy} aria-label="Fechar" onClick={onClose}>
              <X size={16} />
            </WorkshopIconButton>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 border-t border-kumo-line px-5 py-3">
            <WorkshopButton type="button" disabled={busy} onClick={onClose}>Cancelar</WorkshopButton>
...
function OutputsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (loadedOnce.current) {
        toastsRef.current.add({ title: 'Não foi possível atualizar as saídas', variant: 'error' })
      } else {
        setLoadError(true)
      }
    })
  ...
  const saveRename = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Não foi possível renomear esta saída', variant: 'error' })
    } finally {
  ...
  const confirmRemove = async () => {
    ...
    try {
      ...
      toasts.add({ title: 'Não foi possível remover esta saída', variant: 'error' })
    } finally {
      gadget?.[Symbol.dispose]()
      overseer?.[Symbol.dispose]()
      setMutationBusy(false)
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-5xl flex-col px-6 sm:px-10">
      <header className="flex items-end justify-between gap-4 px-3 pb-4 pt-10">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight text-kumo-default">Saídas</h1>
          <p className="mt-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
            Tudo o que seus espaços de trabalho produziram, em um só lugar.
          </p>
        </div>
      ...
      <div className={`flex flex-col gap-3 px-3 pb-3 sm:flex-row sm:items-center sm:justify-between ${
        ...
        <div className="-mx-1 flex items-center gap-1 overflow-x-auto px-1 sidebar-scroll">
          {showTypeFilters && (
            <>
              <FilterChip active={typeFilter === 'all'} label="Todos" count={inTypeScope.length}
      ...
      <div className="chat-panel min-h-0 flex-1 overflow-y-auto pb-8 pt-1">
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">Algo deu errado ao carregar suas saídas.</p>
          ...
          <div className="flex flex-col items-center gap-3 px-3 py-20 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-kumo-default">
                {isFiltered ? 'Nenhuma saída encontrada' : 'Nenhuma saída ainda'}
              </p>
              <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                {isFiltered
                  ? 'Tente um filtro ou termo de busca diferente.'
                  : 'Tudo o que seus espaços de trabalho criarem aparecerá aqui.'}
              </p>
            </div>
      ...
      <DeleteConfirmationDialog
        ...
        title={`Remover “${removeOutput?.title || 'Sem título'}”?`}
        description={
          <>
            Isso remove permanentemente a saída de “{removeOutput?.workspaceTitle}”
            {removeOutput?.owner ? ', para todos com acesso a esse espaço de trabalho' : ''}. As
            outras saídas nesse espaço de trabalho continuam disponíveis. Isso não pode ser desfeito.
          </>
        }
